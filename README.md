...
Active toxin codes and labels are defined in
`backend/samples/constants/mycotoxin_constants.py` (`TOXIN_CHOICES`); optional
threshold metadata lives in `EU_THRESHOLDS`. The result-entry form and dashboard
threshold controller fetch `/api/samples/mycotoxin-registry/`, so catalog changes
no longer require updating their frontend lists. Deploy the backend and include
any Django choices migration generated by `makemigrations` when changing codes.

Removing a code from the active catalog does not delete historical results.
Those results remain readable by code, including on the dashboard and sample
cards. Results without threshold metadata are displayed as unclassified. Public
dashboard snapshots reflect changes after their next publication. The legacy
frontend registry is still used by local import helpers and prediction labels.
