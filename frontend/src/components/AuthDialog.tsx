...
const AuthDialog = () => {
  ...
  const handleGoogleAuth = async () => {
    setUIState(prev => ({ ...prev, isGoogleLoading: true }));
    try {
      // Initialize Google OAuth flow
      window.location.href = `${API_BASE_URL}/accounts/google-auth/`;
    } catch (err) {
      const errorMsg = 'Google authentication failed';
      setUIState(prev => ({ ...prev, error: errorMsg, isGoogleLoading: false }));
    }
  };
        ...
        <div className="w-full flex flex-col p-8 sm:p-10">
          ...
          <div className="flex items-center gap-3 mb-10 justify-center">
            <div 
              ...
              style={{ background: 'linear-gradient(135deg, #00615c 0%, #1d7b75 100%)' }}
            >
              <Leaf className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="text-xl font-bold tracking-tight" style={{ color: '#191c1d' }}>
                AgriScan Pro
              </h1>
          ...
          {uiState.error && (
            <div 
              ...
              role="alert"
            >
              <AlertCircle className="w-5 h-5 mt-0.5 shrink-0" style={{ color: '#ba1a1a' }} />
          ...
          {uiState.successMessage && (
            <div 
              ...
              role="status"
            >
              <Check className="w-5 h-5 mt-0.5 shrink-0" style={{ color: '#00a82c' }} />
          ...
          {mode === 'signup' && (
            <form onSubmit={handleSignUp} className="space-y-4 max-h-96 overflow-y-auto pr-2">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700 transition-colors"
                    aria-label={uiState.showVerifyPassword ? 'Hide password' : 'Show password'}
                  >
              ...
              >
                {uiState.isSubmitting && <Loader2 className="w-4 h-4 animate-spin" />}
                {uiState.isSubmitting ? 'Creating Account...' : 'Create Account'}
              </button>
            </form>
          )}
          ...
          <div className="my-6 flex items-center gap-4">
            <div className="flex-1 h-px" style={{ backgroundColor: '#e1e3e3' }} />
            <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#6e7978' }}>
              Or continue with
            </span>
            <div className="flex-1 h-px" style={{ backgroundColor: '#e1e3e3' }} />
          </div>
