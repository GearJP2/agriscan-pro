...
import { ChevronDown, Share2, Network, ListOrdered, BarChart3, Grid3X3, Activity, Filter } from 'lucide-react';
      ...
      {allToxins.map(toxin => {
        ...
        return (
          <button
            ...
            className={cn(
              "flex items-center justify-between px-4 py-2.5 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all",
              isSelected 
           ...
           <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Detection Analytics & Hazard Patterns</p>
              </div>
            </div>
            <button
          ...
          <CardContent className="px-6 pb-8 pt-8 space-y-12 animate-in fade-in slide-in-from-top-2 duration-500">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-card dark:bg-card rounded-2xl p-8 border-2 border-border dark:border-border/50 flex flex-col md:flex-row items-center justify-between gap-8 group transition-all duration-500 shadow-none">
                <div className="text-center md:text-left space-y-1">
                  <h4 className="text-[13px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-white/50">Most Frequent Pathogenic Cluster</h4>
                  <p className="text-base font-black text-slate-900 dark:text-white uppercase tracking-tight">Co-occurrence pattern signature</p>
                </div>
                  ...
                  {coContamSummary.mostCommonPair.split('+').map((t, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[11px] font-black text-slate-900 dark:text-white group-hover:text-slate-900 dark:group-hover:text-white uppercase tracking-widest">{toxin}</span>
                      </div>
                    );
                  })}
                  {coContamSummary.mostCommonPair === 'None' && (
                    <span className="text-sm font-bold text-white/30 uppercase italic">No active clusters detected</span>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  ...
                  <div key={tile.label} className={cn("rounded-2xl p-10 border-2 bg-card dark:bg-card transition-all hover:border-slate-900 group shadow-none", tile.border)}>
                    ...
                    <p className="text-[12px] font-black uppercase tracking-[0.2em] text-slate-500 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">
                      {tile.label}
                    </p>
                  </div>
                ))}
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                <h3 className="text-[13px] font-black uppercase tracking-[0.2em] text-slate-900 dark:text-white">Common Co-occurrences</h3>
              ...
              <div className="flex flex-wrap gap-6">
                {coOccurrenceList.slice(0, 10).map((co, i) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-black text-teal-600 px-3 py-1 rounded-full bg-teal-50 dark:bg-teal-900/20 uppercase tracking-widest border border-teal-600/20">Rank #{i+1}</span>
                      <span className="text-base font-black text-slate-900 dark:text-white">{co.pct}%</span>
                    </div>
                    <div className="text-[12px] font-black uppercase tracking-tighter text-slate-900 dark:text-white group-hover:text-teal-600 transition-colors leading-tight min-h-[2.5rem] flex items-center">
                      {co.toxins.join(' + ')}
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              ...
              <div className="space-y-5">
                <div className="space-y-2">
                  <h3 className="text-[13px] font-black uppercase tracking-[0.2em] text-slate-900 dark:text-white">Network Strength Analysis</h3>
                  <p className="text-[11px] text-slate-900 dark:text-white/80 font-medium leading-relaxed">
                    Visualizes the co-occurrence relationships between toxins. The thickness of connections represents the statistical probability and strength of finding these toxins together in the same sample.
                  </p>
                </div>
              ...
              <div className="space-y-5">
                <div className="space-y-2">
                  <h3 className="text-[13px] font-black uppercase tracking-[0.2em] text-slate-900 dark:text-white">Burden Distribution (Toxins per Sample)</h3>
                  <p className="text-[11px] text-slate-900 dark:text-white/80 font-medium leading-relaxed">
                    Illustrates the frequency of multiple mycotoxicosis, showing how many different toxins are present simultaneously in positive samples.
                  </p>
                </div>
