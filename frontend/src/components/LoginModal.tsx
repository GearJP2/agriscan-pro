...
const LoginModal = () => {
  ...
  const handleSignUpSubmit = async (e: FormEvent) => {
    ...
    if (!passwordRegex.test(password)) {
      setError(
        "Password must be at least 8 characters long, contain at least uppercase, lowercase, and a number.",
      );
      return;
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        <Button variant="ghost" className="h-auto gap-2 rounded-none px-1.5 py-2 text-xs font-semibold text-[#365247] hover:bg-transparent hover:text-[#9a6b37] dark:text-slate-300 dark:hover:text-[#e5c987]">
          ...
          <div className="hidden justify-center mb-8">
            <div className="bg-muted p-1 rounded-full inline-flex relative shadow-inner">
              <button
                ...
                className={`relative px-6 py-2 text-sm font-semibold rounded-full transition-all duration-300 z-10 ${
                  mode === "signin"
              ...
              {/* Sliding background */}
              <div
                className={`absolute top-1 bottom-1 w-1/2 bg-primary rounded-full transition-transform duration-300 ease-in-out shadow-sm ${
                  mode === "signin"
                    ? "left-1 translate-x-0"
                    : "left-[-4px] translate-x-full"
                }`}
              />
            </div>
          </div>
          ...
          <div className="mt-8 pt-6 border-t border-border flex flex-col items-center gap-4">
            ...
            <button
              ...
              className="w-full h-11 rounded-xl flex items-center justify-center gap-3 border-2 border-border text-foreground font-semibold text-sm hover:bg-muted/50 transition-colors duration-200
                            hover:border-primary/20 hover:shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"
