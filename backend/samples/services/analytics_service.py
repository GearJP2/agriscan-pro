...
class AnalyticsService:
    ...
    def get_overview(filters: dict) -> dict:
        ...
        return {
            'kpis': {
                ...
                'positive_pct': round(
                    (above_threshold_samples / total_samples * 100),
                    1,
    ...
    def get_co_contamination(filters: dict) -> dict:
        ...
        # Cross-combinations are clearer in Python than in raw SQL/CTEs here.
        co_occurrences = defaultdict(int)
        toxins_per_sample = {1: 0, 2: 0, 3: 0, 4: 0}
        ...
        for combo, count in sorted(
            ...
            combinations.append({
                ...
                'sampleCount': count,
                'pct': (
                    round((count / positive_count * 100), 1)
                    if positive_count > 0 else 0
                )
            })
        ...
        links = [
            ...
            for k, v in network_edges.items()
        ]
    ...
    def simulate_threshold(overrides: dict, filters: dict) -> dict:
        ...
        positive_by_province = defaultdict(int)
        ...
        return {
            'kpis': {
                'total_samples': total_samples,
                'positive_pct': round(
                    (total_above / total_samples * 100),
                    1,
                ) if total_samples > 0 else 0,
                'detected_pct': round(
                    (
                        qs.filter(mycotoxin_results__isnull=False).distinct().count()
                        / total_samples
                        * 100
                    ),
                    1,
            ...
            'provinces': [
                {
                    ...
                    'positiveCount': positive_by_province[p['province']],
                    'positivePct': round(
                        (
                            positive_by_province[p['province']]
                            / above_by_province_total[p['province']]
                            * 100
                        ),
                        1,
                    ) if above_by_province_total[p['province']] > 0 else 0,
                    'aboveThresholdPct': round(
                        (
                            above_by_province[p['province']]
                            / above_by_province_total[p['province']]
                            * 100
                        ),
                        1,
