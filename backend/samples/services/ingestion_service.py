...
class SampleIngestionService:
    ...
    def get_row_value(cls, row, candidate_keys):
        normalized_map = {cls.normalize_token(k): v for k, v in (row or {}).items() if k is not None}
    ...
    def extract_row_sample_id(cls, row):
        key_groups = [['sample_id', 'sample id', 'sampleid', 'sample code'], ['name'], ['sample'], ['']]
        candidates = []
        ...
        for keys in key_groups:
            ...
            if not norm or norm in {'NAME', 'SAMPLE', 'DATE', 'DATETIME', 'ACQDATETIME'}: continue
    ...
    def extract_analyzed_datetime(cls, row):
        explicit = cls.get_row_value(row, [
            'analyzed_at', 'analyzed at', 'analysis_datetime', 'analysis datetime',
            'acq. date-time', 'acq date-time', 'acq_datetime', 'datetime'
        ])
    ...
    def extract_results_from_row(cls, row):
        ...
        toxin_key = cls.get_row_value(row, ['mycotoxin', 'mycotoxin_name', 'toxin', 'name'])
        if toxin_key:
            canonical = cls.resolve_toxin_name(toxin_key)
            intensity = cls.parse_numeric(cls.get_row_value(row, ['intensity', 'value', 'result', 'concentration']))
            if intensity is None:
                return []

            threshold = TOXIN_DEFAULTS.get(canonical, {}).get('threshold', 0.0)
            unit = TOXIN_DEFAULTS.get(canonical, {}).get('unit', 'ppb')
        ...
        for row in rows:
            ...
            for payload in results:
                ...
                if existing:
                    ...
                    created_row = MycotoxinResult.objects.create(
                        sample=sample,
                    ...
                    cls.cleanup_legacy_duplicates(sample, payload['name'], created_row.id)
                    created += 1
